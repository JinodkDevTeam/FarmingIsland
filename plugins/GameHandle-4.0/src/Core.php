...
class Core extends PluginBase{
	public const VERSION = "0.2.7-alpha";
	...
	/*public function makeJson() : void{
		...
		if(!file_exists($this->getDataFolder() . "output/")){
			mkdir($this->getDataFolder() . "output");
		}
		$name_file = fopen($this->getDataFolder() . "output/name.lang", "w");
		$register_file = fopen($this->getDataFolder() . "output/register.php", "w");
		if(!$name_file){
			$this->getLogger()->error("Cannot open file name.lang");
			return;
		}
		if(!$register_file){
			$this->getLogger()->error("Cannot open file register.php");
			return;
		}
		...
		foreach($list_files as $full_file_name){
			if(str_contains($full_file_name, ".png")){
				...
				$class_file = fopen($this->getDataFolder() . "output/" . $fish_class_name . ".php", "w");
				if(!$class_file){
					$this->getLogger()->error("Cannot open file " . $fish_class_name . ".php");
					return;
				}
				$properties = [
					"<?php",
